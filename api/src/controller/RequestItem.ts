...
class RequestItemController {
  async create(request: Request, response: Response) {
    ...
    if (!solicitationNumber) {
      return response.status(400).send({ Mensagem: "Solicitação inexistente" });
    }
    ...
    const materialAlreadyExist =
      await RequestItemController.checkMaterialAlreadyExist(
        solicitationId,
        materialId
      );
  ...
  async update(request: Request, response: Response) {
    ...
    const newMaterialId = await IdFinder.findMaterialId(Number(materialCode));
    ...
    const isMaterialOnSolicitation =
      await RequestItemController.findMaterialOnSolicitation(
        solicitationId,
        currentMaterialId
      );
    ...
    const materialAlreadyExist =
      RequestItemController.checkMaterialAlreadyExist(
        solicitationId,
        newMaterialId
      );
    if (materialAlreadyExist) {
  ...
  async delete(request: Request, response: Response) {
    ...
    if (!materialId) {
      return response.status(400).send({ Mensagem: "Item não encontrado" });
    }
  ...
  static async checkMaterialAlreadyExist(
